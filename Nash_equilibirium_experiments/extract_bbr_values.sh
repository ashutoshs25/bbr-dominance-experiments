...
echo "N_bdp,flow,trial,bits_per_second" > bbr_results.csv
...
for i in "${N_bdp[@]}"; do
  for j in $(seq 0 "$((n_flows))"); do
    for k in $(seq 1 "$trials"); do
      ...
      bbr_data=$(<"bbr-result-$i-$j-$k.json")

      # Extract the value from the JSON
      value=$(echo "$bbr_data" | jq -r '.end.sum_received.bits_per_second')

      # Append the values to the CSV file
      echo "$i,$j,$k,$value" >> bbr_results.csv
    done
  done
done
